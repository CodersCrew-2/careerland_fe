    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#0f1117', borderRadius: 16, overflow: 'hidden' }}>
            ...
            <div style={{ background: '#1a1d27', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                ...
                <span style={{ color: '#8b90a5', fontSize: 13, marginLeft: 'auto', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {graph.title}
                </span>
                {onRegenerate && (
                    <button
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden', minHeight: 0 }}>
                {/* Left node list */}
                <div style={{ width: 240, minWidth: 240, background: '#1a1d27', borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    ...
                    <ul style={{ listStyle: 'none', overflowY: 'auto', flex: 1, padding: '6px 0', margin: 0 }}>
                        {graph.nodes.map(node => (
                            ...
                            >
                                <div style={{ fontSize: 13, fontWeight: 500, color: '#e1e4ed', marginBottom: 4 }}>{node.label}</div>
